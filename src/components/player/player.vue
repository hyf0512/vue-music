          ...
          <scroll class="middle-r" ref="lyricList" :data="currentLyric && currentLyric.lines">
            <div class="lyric-wrapper">
              <div v-if="currentLyric">
                <p ref="lyricLine" class="text" v-for="(line, index) in currentLyric.lines" :key="line.key" :class="{'current': currentLineNum === index}">{{line.txt}}</p>
              </div>
            </div>
          </scroll>
        </div>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      enter(el, done) {
        const {x, y, scale} = this._getPoseandScale()
